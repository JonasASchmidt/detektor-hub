...
export async function GET(request: Request) {
    ...
    if (Number.isNaN(parsedLat) || Number.isNaN(parsedLon)) {
        return new Response(JSON.stringify({ error: 'lat and lon need to be valid numbers' }), { status: 400 })
    }

    if (parsedLat < -90 || parsedLat > 90) {
        return new Response(JSON.stringify({ error: 'lat needs to be between -90 and 90' }), { status: 400 })
    }

    if (parsedLon < -180 || parsedLon > 180) {
        return new Response(JSON.stringify({ error: 'lon needs to be between -180 and 180' }), { status: 400 })
    }
